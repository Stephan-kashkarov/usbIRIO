...
int main(int argc, char **argv)
{
	...
	{
		device = bus->devices;
		cout << "Device Vendor: " << hex << device->descriptor.idVendor << " Device ID: "<< hex << device->descriptor.idProduct << endl;
		while (device != NULL)
		{
			// Check to see if each USB device matches the DigiSpark Vendor and Product IDs
			if ((device->descriptor.idVendor == 0x16d0) && (device->descriptor.idProduct == 0x753))
			{
				if (debug)
		...
		if (argc > 1)
		{
           ...
           if(result < 0) {if(debug) printf("Error %i claiming Interface %i\n", result, interface->bInterfaceNumber); return 1;}*/
				...
				while (i < stringLength)
				{
					...
					{
						if (debug)
							printf("Writing new line to DigiSpark.\n");
